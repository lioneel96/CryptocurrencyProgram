...
def creaTabla():
    ...
    for i in range(len(monedas)):
        for j in range(5):
            ...
            variables[(5*i)+j].set("0")
    
    for i in range(len(monedas)):
        for j in range(5):
            entries.append( Entry(miFrame, textvariable=variables[(5*i)+j]) )
            entries[(5*i)+j].grid(row=i+1,column=j+1,padx=10,pady=5)
            entries[(5*i)+j].config(justify="center")            
...
def modificaCeldas():
    for i in range(len(datos)):
        if(datos[i][4]>3):
            ...
            #entries[(5*i)+4].config(background="palegreen")
...
def salir():
    rta=messagebox.askquestion("Salir", "¿Desea salir de la aplicacion?")
    if rta=="yes" :
        raiz.destroy()
...
Label(firstFrame, text="Cryptocurrency values and data, by L&M").grid(row=0,column=0,padx=10,pady=5,columnspan=2)
